...
main(int argc, char *argv[])
{
  ...
  for(i = 1; i < argc; i++){
    if(mkdir(argv[i]) < 0){
      printf(2, "mkdir: %s failed to create\n", argv[i]);
  44:	e3001b88 	movw	r1, #2952	; 0xb88
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 754:	e3008ba4 	movw	r8, #2980	; 0xba4
    neg = 1;
    x = -xx;
...
printf(int fd, char *fmt, ...)
{
 ...
 870:	0a00002d 	beq	92c <printf+0xd8>
        ...
        if(s == 0)
          s = "(null)";
 874:	e3008bb8 	movw	r8, #3000	; 0xbb8
...
free(void *ap)
{
 ...
 9ec:	e300cbc0 	movw	ip, #3008	; 0xbc0
void
free(void *ap)
{
  Header *bp, *p;
...
malloc(uint nbytes)
{
 a88:	e92d4bf0 	push	{r4, r5, r6, r7, r8, r9, fp, lr}
  ...
  if((prevp = freep) == 0){
 a8c:	e3008bc0 	movw	r8, #3008	; 0xbc0
...
malloc(uint nbytes)
{
 ...
 ac0:	e3540a01 	cmp	r4, #4096	; 0x1000
    ...
    if(p == freep)
 ac4:	e3005bc0 	movw	r5, #3008	; 0xbc0
